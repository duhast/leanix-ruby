  ...
  class AccessControlEntity
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LeanixApi::AccessControlEntity` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LeanixApi::AccessControlEntity`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@name.nil? && @name.to_s.length > 2000
        invalid_properties.push('invalid value for "name", the character length must be smaller than or equal to 2000.')
      end
      ...
      if !@display_name.nil? && @display_name.to_s.length > 2000
        invalid_properties.push('invalid value for "display_name", the character length must be smaller than or equal to 2000.')
      end
    ...
    def valid?
      ...
      return false if !@name.nil? && @name.to_s.length > 2000
      return false if !@name.nil? && @name.to_s.length < 0
      return false if !@display_name.nil? && @display_name.to_s.length > 2000
    ...
    def name=(name)
      ...
      if name.to_s.length > 2000
        fail ArgumentError, 'invalid value for "name", the character length must be smaller than or equal to 2000.'
      end
    ...
    def display_name=(display_name)
      ...
      if display_name.to_s.length > 2000
        fail ArgumentError, 'invalid value for "display_name", the character length must be smaller than or equal to 2000.'
      end

      if display_name.to_s.length < 0
    ...
    def hash
      [id, name, display_name, description].hash
    end
